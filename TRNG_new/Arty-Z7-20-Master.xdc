...
## Switches
#set_property -dict { PACKAGE_PIN M20    IOSTANDARD LVCMOS33 } [get_ports { enable_sw_0 }]; #IO_L7N_T1_AD2N_35 Sch=SW0
#set_property -dict { PACKAGE_PIN M19    IOSTANDARD LVCMOS33 } [get_ports { enable_sw }]; #IO_L7P_T1_AD2P_35 Sch=SW1
...
## LEDs
#set_property -dict { PACKAGE_PIN R14    IOSTANDARD LVCMOS33 } [get_ports { led_debug[0] }]; #IO_L6N_T0_VREF_34 Sch=LED0
#set_property -dict { PACKAGE_PIN P14    IOSTANDARD LVCMOS33 } [get_ports { led_debug[1] }]; #IO_L6P_T0_34 Sch=LED1
#set_property -dict { PACKAGE_PIN N16    IOSTANDARD LVCMOS33 } [get_ports { led_debug[2] }]; #IO_L21N_T3_DQS_AD14N_35 Sch=LED2
#set_property -dict { PACKAGE_PIN M14    IOSTANDARD LVCMOS33 } [get_ports { led_debug[3] }]; #IO_L23P_T3_35 Sch=LED3

## Buttons
#set_property -dict { PACKAGE_PIN D19    IOSTANDARD LVCMOS33 } [get_ports { rst_btn_0 }]; #IO_L4P_T0_35 Sch=BTN0
#set_property -dict { PACKAGE_PIN D20    IOSTANDARD LVCMOS33 } [get_ports { btns[1] }]; #IO_L4N_T0_35 Sch=BTN1
#set_property -dict { PACKAGE_PIN L20    IOSTANDARD LVCMOS33 } [get_ports { btns_4bits_tri_i[2] }]; #IO_L9N_T1_DQS_AD3N_35 Sch=BTN2
#set_property -dict { PACKAGE_PIN L19    IOSTANDARD LVCMOS33 } [get_ports { rst_0 }]; #IO_L9P_T1_DQS_AD3P_35 Sch=BTN3
...
#set_property -dict { PACKAGE_PIN P18   IOSTANDARD LVCMOS33 } [get_ports { uart_txd }]; #IO_L23N_T3_34           Sch=CK_IO12
...
# Allow combinatorial loops for all 8 ring oscillators in TRNG

# ==== Giữ nguyên hierarchy + cell osc để hạn chế đổi tên ====
set_property KEEP_HIERARCHY yes [get_cells -hier -filter {NAME =~ *osc_cell*}]
set_property DONT_TOUCH    true [get_cells -hier -filter {NAME =~ *osc_cell*}]

# ==== Cho phép vòng combinational trong toàn thiết kế (an toàn thêm) ====
# (Không bắt buộc, nhưng hữu ích ở một số phiên bản Vivado)
#catch { set_property HD.COMBINATIONAL_LOOPS TRUE [current_design] }

# ==== Cho phép loop ngay trên các net "đúng tên" trong osc_cell ====
# (áp dụng khi Vivado còn giữ được tên x1/x2/a1/a2)
set_property ALLOW_COMBINATORIAL_LOOPS TRUE [get_nets -hier -regexp {.*osc_cell.*/(xor1|xor2|and1|and2)(\[.*\])?$}]

#set_property LOC SLICE_X0Y0 [get_cells {u_trng/u_core/u_osc_0}]
#set_property LOC SLICE_X2Y0 [get_cells {u_trng/u_core/u_osc_1}]
#set_property LOC SLICE_X4Y0 [get_cells {u_trng/u_core/u_osc_2}]
#set_property LOC SLICE_X6Y0 [get_cells {u_trng/u_core/u_osc_3}]

set_property CLOCK_DEDICATED_ROUTE FALSE [get_nets {u_trng/u_core/osc_jitter[*]}]
# 1. Cho phép Loop trên MỌI dây bên trong osc_cell
# (Dùng .* để bắt cả tên gốc lẫn tên bị đổi như _inferred_)
set_property ALLOW_COMBINATORIAL_LOOPS TRUE [get_nets -hier -regexp {.*osc_cell.*/.*}]

#set_property ALLOW_COMBINATORIAL_LOOPS TRUE [get_nets -hier -filter {NAME =~ *u_osc_0/and1*}]
#set_property ALLOW_COMBINATORIAL_LOOPS TRUE [get_nets -hier -filter {NAME =~ *u_osc_0/and2*}]
#set_property ALLOW_COMBINATORIAL_LOOPS TRUE [get_nets -hier -filter {NAME =~ *u_osc_0/xor1*}]
#set_property ALLOW_COMBINATORIAL_LOOPS TRUE [get_nets -hier -filter {NAME =~ *u_osc_0/xor2*}]
# 2. Tắt kiểm tra timing cho các dây này (vì nó dao động tự do)
set_false_path -through [get_nets -hier -regexp {.*osc_cell.*/.*}]

# 3. [QUAN TRỌNG NHẤT] Hạ cấp lỗi DRC LUTLP-1 từ ERROR xuống WARNING
# Lệnh này bắt buộc Vivado phải nhắm mắt cho qua để tạo Bitstream

# ==== (Khuyến nghị) Chặn timing qua các net loop để tool khỏi "cứu hộ" tối ưu ====
set_false_path -through [get_nets -hier -regexp {.*osc_cell.*/(xor1|xor2|and1|and2)(\[.*\])?$}]

# ==== Trường hợp Vivado đổi tên về dạng *_inferred_i_* (wildcard dự phòng) ====
# Lưu ý: pattern này khá rộng; dùng để "vét" nốt các net còn sót.
#set_property ALLOW_COMBINATORIAL_LOOPS TRUE [get_nets -hier -regexp {.*_inferred_i_.*}]
# (tùy bạn) chặn timing qua các net inferred
#set_false_path -through [get_nets -hier -regexp {.*_inferred_i_.*}]
set_property SEVERITY {Warning} [get_drc_checks LUTLP-1]

set_false_path -through [get_pins -hier -filter {NAME =~ *u_fifo/mem*/CLKARDCLK}]

set_false_path -from [get_cells -hier -filter {NAME =~ *u_von/first_bit*}] \
               -to [get_cells -hier -filter {NAME =~ *u_von/bit_out*}]
